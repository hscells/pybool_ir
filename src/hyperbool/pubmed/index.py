...
def load_index(path: Path) -> engine.Indexer:
    ...
    indexer.set("publication_type", engine.NestedField, sep="|", stored=True)
    indexer.set("mesh_heading_list", engine.NestedField, sep="|", stored=True)
    indexer.set("keyword_list", engine.NestedField, sep="|", stored=True)
    return indexer
